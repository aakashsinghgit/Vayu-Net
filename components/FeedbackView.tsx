...
const FeedbackView: React.FC<FeedbackViewProps> = ({ zone, userRole, onRequestLogin }) => {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 animate-in fade-in">
        ...
        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">Thank you for your feedback!</h3>
        <p className="text-slate-500 dark:text-slate-400 mb-8">Your input helps us improve air quality monitoring in {zone.name}.</p>
        <button 
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-in fade-in duration-500">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Community Feedback</h2>
        <p className="text-slate-500 dark:text-slate-400">Report issues or suggest improvements for {zone.name}</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
        ...
        {userRole === 'GUEST' && (
             <div className="absolute top-0 right-0 p-4">
                 <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full text-xs font-bold border border-slate-200 dark:border-slate-700">
                    <Lock size={10} />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full h-40 p-4 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 resize-none"
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-xl border border-yellow-100 dark:border-yellow-900/30 flex items-start gap-3">
        ...
        <div>
          <h4 className="font-semibold text-yellow-800 dark:text-yellow-400 text-sm">Emergency Notice</h4>
